...
| Server alerts (staff) | `fetchAdminAlerts` / `acknowledgeAdminAlert` / `unacknowledgeAdminAlert` / `resolveAdminAlert`. Alerts are raised by the hosts themselves (map switch failed, anti-cheat did not start, map vote data not serving) and by the backend's own polling (host unreachable, server process down). They are deduplicated server-side: one open alert per target, with `occurrences` counting repeats - never render each occurrence as a separate row. `status` is `open` / `acknowledged` / `resolved`; resolving is a claim it was dealt with, not a fix, and a recurrence opens a fresh alert, so resolving early costs nothing. Ack and resolve are audited, and resolve takes a reason |
| Host metrics + health events (staff) | `fetchAdminHostMetrics(token, hostId, window)` returns `{window, bucket, points[]}` where `bucket` is `sample` for 24h and `hour` for longer windows - hourly points are averages, so short spikes are smoothed away and the UI says so rather than implying the spike never happened. `fetchAdminHealthEvents` is the append-only transition feed (`host.unreachable`, `host.recovered`, `server.process_down`, `server.process_up`). A server carries TWO independent liveness readings: `probe` (does it answer players) and `runtime` (is the process alive on its host). Render both and never reconcile them - `runtime.process_up` true with `probe.answering` false is a HUNG server, and that disagreement is the whole point. `runtime.known` false means it has never been polled and `runtime.stale` true means the poller has not been round recently; neither is the same as down, so do not draw them as down |
| Server operations (staff) | `fetchAdminServerOperations` returns every registered game server with the host it is linked to and a `probe` block (`answering`, `map_name`, `num_players`, `max_players`, `updated_at`, `seconds_stale`). `answering` is computed server-side - the launcher must not re-derive it from `seconds_stale`. A server with no `host_ref` is not linked to a host yet and is surfaced as such rather than hidden. Moderators and admins both read this; only admins see the Game Hosts section next to it |
