...
| `utbt:teamsState:v1` | `Main.tsx` (`usePageState`) | none persisted (no pref fields); per-entry query state = team-directory search/open-only/page + scroll. In-team data (`myTeam`/invitations/applications) lives in the shared caches singleton, refreshed on mount and updated from each mutation's fresh `TeamDetail`. |
