...
| `utbt:teamsState:v1` | `Main.tsx` (`usePageState`) | none persisted (no pref fields); per-entry query state = team-gallery search/page + scroll. Gallery caches (`myTeam`, invitations) live in the shared singleton, refreshed on mount and updated from each mutation's fresh `TeamDetail`. The role-aware `team-detail` page fetches its own `/teams/<id>` + `/me/team`; leave/disband/apply invalidate the gallery cache and route back. |
