...
| `PoolGrid` / `PoolCard` | `cards`, `previewActor` (the side whose selection preview is shown), `className?` | Square `MapThumbnail` cards. Banned: grey with a "BANNED" stamp in the banning side's colour. Picked: the picker's colour and "Map N". Decider: gold. Excluded: faded, "Excluded · <tag> maps", and the full reason in the title and screen-reader text. The stamp and chips enter and exit, and the grey and border colour transition, so an undo reverses them. Nothing animates on first render. |
