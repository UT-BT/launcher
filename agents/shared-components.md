...
| `PickBanMotion` | `animate`, `children` | Wraps any tree of the visual core (the match page, the stream view). A framer-motion `MotionConfig` driven by `animate`, not by the OS reduced-motion switch: pick/ban reveals are the show, and Windows reports "reduce" whenever its Animation effects are off. The match page takes `animate` from its Animations toggle (`pickBanMotionPreference.ts`, key `utbt:pickBanMotion:v1`, on by default); the stream view animates unless its URL carries `motion=0`. With `animate` off, every enter, exit and layout animation below it is instant and the information shown is the same. Its wrapper carries `data-motion="on"` while animating, which exempts the subtree from `shared.css`'s reduced-motion rule. `usePickBanAnimate()` reads the value below it. |
| `PickBanSoundControl` | `on`, `preference` (`{ pack, volume }`), `onToggle`, `onChange` (the whole next preference), `onPreview(pack)`, `className?` (the trigger's classes) | The match page's header sound control: a button showing the state ("Sound on"/"Sound off") that opens a small panel with a Sound switch (`ui/switch`, labelled by its row), a Style segmented `radiogroup` (Cinematic / Clean, each with a one-line hint; arrow keys move and choose, like the manager dock's "Who is Team A?") and a Volume `ui/slider` 0–100% with the number shown. A style change calls `onChange` then `onPreview`; the slider keeps a local draft while it moves and calls both only on release (pointer up, key up or blur). It is a disclosure (`aria-expanded`, a labelled group under the button), not a Radix menu, so Tab and the arrow keys reach the switch, radios and slider; Escape, an outside pointer or focus leaving closes it. 44px rows below `sm`. It holds no sound state itself: the page wires it to `usePickBanSound` and `pickBanSoundPreference.ts` (see `agents/navigation.md` → `match-pickban-page`). |
