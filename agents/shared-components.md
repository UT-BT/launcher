...
| `usePickBanPreload(cards)` | `view?.cards`; returns `true` once settled | Fetches and decodes every non-excluded pool screenshot at the `card` size the core renders (following `MapThumbnail`'s derived → canonical → default fallback), and loads the fonts. It holds the decoded images while mounted and keys on the list of URLs, so rebuilding the view doesn't refetch anything. Call it from the lobby on and when the stream view loads. |
