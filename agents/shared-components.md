...
|---|---|---|
| `TeamPanel` | `panel: PickBanTeamPanel \| null`, `className?` | The A/B chip, stage seed chip, team name, a reserved "Contemplating…" row, and every member through `PlayerInfo` (with their selected title) with a presence dot and a captain or acting-captain badge. Tinted and ringed in the side's colour. |
