...
| `app/utils/format.ts` | `formatCapTime` (`MM:SS.mmm`), `formatDelta`, `formatAddedDate`, `formatTimeAgo` (relative; renders `in 2h` rather than lying with `just now` when a stamp is ahead of the clock), `isNew` (30-day window), `displayMapName` (strips `CTF-BT-` / `CTF-BT+` prefix), and `parseApiDate`. **New code must parse every API date with `parseApiDate`** — it reads space-separated, `T`-separated, date-only, offset-bearing and RFC-1123 forms, and treats a zone-less string as UTC. A raw `new Date(apiValue)` reads a zone-less string as *local* time and is wrong by the viewer's offset. Roughly thirty call sites still parse raw and are being migrated — `LeaderboardCard.tsx` even renders the same `added` field both ways (`formatAddedDate` at :226, `new Date` at :346) — so don't read an existing `new Date(...)` as a blessed pattern. |
