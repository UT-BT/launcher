...
| data-sources | `agents/data-sources.md` | client-side API contract + asset URLs + favorites/patreon sync + the event bracket/format contract | `app/utils/api.ts`, `app/utils/patreon.ts`, `app/utils/server-utils.ts`, `app/hooks/useServerFavorites.ts`, `app/components/pages/events/manage/formatFields.tsx` |
