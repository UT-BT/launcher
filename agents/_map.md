...
| shared-components | `agents/shared-components.md` | inventory of reusable components + when to use each | `app/components/shared/{PlayerInfo,DataTable,CapTimeLink,MapNavLink,MapNameCell,ColumnsMenu,FilterPresetsMenu}.tsx`, `app/components/shared/{capTimeTarget,runDemo,runDemoLabels}.ts` |
...
| data-sources | `agents/data-sources.md` | client-side API contract + asset URLs + favorites/patreon sync + the event bracket/format contract | `app/utils/api.ts`, `app/components/shared/runDemo.ts`, `app/hooks/{useReplayWatch,replayMessages,useDemoDownload}.ts`, `app/utils/patreon.ts`, `app/utils/server-utils.ts`, `app/components/pages/events/manage/formatFields.tsx` |
