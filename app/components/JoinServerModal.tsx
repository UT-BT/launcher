...
export function JoinServerModal({ server, isOpen, onClose, onJoin, installationStatus }: JoinServerModalProps) {
    ...
    useEffect(() => {
        ...
        // Focus first element when modal opens, or modal itself if no focusable elements
        if (firstElement) {
            firstElement.focus()
        } else {
            modalElement.focus()
    ...
    return (
        <div className="fixed top-[var(--window-titlebar-height)] right-0 bottom-0 left-64 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div ref={modalRef} tabIndex={-1} className="w-full max-w-xl bg-card border border-white/10 rounded-xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
                {/* Header */}
