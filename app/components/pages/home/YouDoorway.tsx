...
export function YouDoorway({ userId, alias, title, accessToken, className, refreshKey }: YouDoorwayProps) {
    const [worldRecords, setWorldRecords] = useState<number | null>(null)

    useEffect(() => {
        if (!accessToken || !userId) return
        let cancelled = false
        fetchUserSummary(accessToken, userId)
            .then(s => { if (!cancelled) setWorldRecords(s.medals.world_records) })
            .catch(() => {})
